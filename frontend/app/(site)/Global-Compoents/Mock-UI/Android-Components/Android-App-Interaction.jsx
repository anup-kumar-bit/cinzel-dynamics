  ...
  if (screens.length === 0) {
    return (
      <div className="android-wake-in absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[#1f1f23] px-6 text-center">
        ...
        <p className="font-opensans text-[10px] text-white/60">No screens added to {title} yet.</p>

        {/* The nav bar alone is too small to read as a way out of a dead end. */}
        <button
          type="button"
          onClick={onClose}
          className="font-opensans mt-1 flex cursor-pointer items-center gap-1 rounded-full bg-[#a8c7fa] px-3 py-1.5 text-[10px] font-semibold text-[#0b2b62] transition hover:brightness-105 active:scale-95"
        >
          <span aria-hidden="true" className="icon-[lucide--arrow-left] size-3" />
          Back
        </button>
